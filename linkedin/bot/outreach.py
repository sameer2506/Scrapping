...
class Messages:
    ...
    def check_user(self, message, url):
        ...
        try:
            ...
            if buttons[0].lower() == "message" or buttons[0].lower() == "follow":
                ...
                self.driver.find_element_by_css_selector(CSS_SELECTOR['connect_button']).click()
                time.sleep(self.time)
    ...
    def inside_connect_button(self, request_sent, message):
        ...
        try:
            self.driver.find_element_by_css_selector(CSS_SELECTOR['connect_inside_button']).click()
            time.sleep(self.time)
            request_sent = self.add_note(request_sent, message)
            return request_sent
    ...
    def add_note(self, request_sent, message):
        ...
        try:
            # add note
            self.driver.find_element_by_css_selector(CSS_SELECTOR['add_note_button']).click()

            note_area = self.driver.find_element_by_css_selector(CSS_SELECTOR['note_area'])
            time.sleep(self.time)
            # print(message)
            ...
            # send button
            self.driver.find_element_by_css_selector(CSS_SELECTOR['send_button']).click()
            request_sent = True
            time.sleep(self.time)
            return request_sent
        except Exception as ex:
    ...
    def run(self):
        ...
        profile_list = LinkedInProfile.objects.filter(~Q(status="messageSent"))
